...
async def create_notice(
    ...
    if notice_data.coordinator_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only create notices for yourself",
        )
...
@router.get("/coordinator/{coordinator_id}", response_model=List[NoticeSchema])
async def get_notices_by_coordinator(
    ...
    if (
        current_user.user_type != UserType.COORDINATOR
        and current_user.id != coordinator_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Access denied"
        )

    notices = await NoticeService.get_notices_by_coordinator(db, coordinator_id)
    return notices
